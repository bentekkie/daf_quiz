...
export function ShareButtons({ scorePercentage, dafRef }: ShareButtonsProps) {
    ...
    const twitterShareUrl = `https://twitter.com/intent/tweet?text=${encodedText}&url=${encodedUrl}&hashtags=DafYomi,Talmud,DafQuizzer`;
    ...
    const handleCopyToClipboard = () => {
        ...
        navigator.clipboard.writeText(`${shareText} ${shareUrl}`)
            .then(() => {
                // Per guidelines, toast is only for errors.
                // A visual cue could be added here if desired.
  ...
  return (
    <div className="flex items-center gap-2">
        ...
        <Button variant="outline" size="icon" asChild>
            <a href={facebookShareUrl} target="_blank" rel="noopener noreferrer" aria-label="Share on Facebook"><FacebookIcon /></a>
        </Button>
