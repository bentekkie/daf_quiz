...
export function ShareButtons({ scorePercentage, dafRef, quizType }: ShareButtonsProps) {
    const { toast } = useToast();
    const quizUrl = typeof window !== 'undefined' ? window.location.href : '';
    const sefariaUrl = `https://www.sefaria.org/${dafRef.replace(/ /g, '_')}`;

    const shareText = `I just took the daily ${quizType} quiz for ${dafRef}! Test your knowledge on Daf Quiz. You can study ${quizType} on Sefaria: ${sefariaUrl}`;
    const twitterShareText = `I just took the daily ${quizType} quiz for ${dafRef}! Test your knowledge here. #${quizType.replace(/ /g, '')}`;
